...
def set_optimizer(model, lr_base, lr_mult=opts['lr_mult'], momentum=opts['momentum'], w_decay=opts['w_decay']):
    ...
    optimizer = optim.SGD(
        param_list, lr=lr, momentum=momentum, weight_decay=w_decay)
    return optimizer
...
def train():
    ...
    model = SiamFC(opts['init_model_path'], K)
    if opts['use_gpu']:
        model = model.cuda()
    ...
    for i in range(opts['n_cycles']):
        ...
        if cur_prec > best_prec:
            ...
            print("Save model to %s" % opts['save_model_path'])
            torch.save(states, opts['save_model_path'])
            if opts['use_gpu']:
                model = model.cuda()
